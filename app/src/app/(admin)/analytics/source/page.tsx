  ...
  return (
    <>
      <PageHeader title="来源分析" description="游客来源渠道分布"
        actions={
          <a href="/api/export/source" download className="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-3 py-2 text-sm font-medium text-foreground hover:bg-muted">
      ...
      <div className="mb-5 grid grid-cols-2 gap-4">
        <div className="rounded-lg border border-border bg-card p-4">
          <p className="mb-3 text-[13px] font-medium text-foreground">渠道分布</p>
          <BarList height={300} emptyText="暂无渠道数据" data={rows.map((r) => ({ label: r.channel, value: r.count, hint: `${r.percentage}%` }))} />
        </div>
        <div className="rounded-lg border border-border bg-card p-4">
          <p className="mb-3 text-[13px] font-medium text-foreground">渠道说明</p>
      ...
      <div className="mb-5">
        <p className="mb-2 text-[13px] font-medium text-foreground">来源地区分布(行政区划下钻)</p>
        <RegionMap provinceData={provinceData} />
      </div>

      <div className="rounded-lg border border-border bg-card">
        <div className="border-b border-border px-4 py-3"><p className="text-[13px] font-medium text-foreground">渠道明细</p></div>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {rows.length === 0 ? (
              ...
              <TableRow key={r.channel} className="hover:bg-muted">
                <TableCell className="text-[13px] text-foreground">{r.channel}</TableCell>

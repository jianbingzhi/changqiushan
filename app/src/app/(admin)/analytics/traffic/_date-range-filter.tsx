    ...
    <form method="GET" className="flex items-end gap-3 flex-wrap">
      {fields.map((f) => (
        <div key={f.name} className="flex flex-col gap-1">
          <label className="text-[13px] text-muted-foreground" htmlFor={f.name}>
            {f.label}
          </label>
      ...
      <button
        type="submit"
        className="h-9 rounded-md bg-primary px-4 text-sm font-medium text-white hover:opacity-90"
      >
        查询
      </button>
    </form>

...
export function SortableContentRows({
    ...
    <Table>
      <TableHeader>
        <TableRow className="bg-[#F9FAFB]">
          {cols.map((h, i) => (
            <TableHead key={i} className="text-xs font-semibold text-[#6B7280]">{h}</TableHead>
          ))}
        </TableRow>
      </TableHeader>
      <TableBody>
        {sortable ? (
          ...
          rows.map((row) => (
            <TableRow key={row.id} className="bg-white hover:bg-[#F9FAFB]">
  ...
  return (
    <div className="rounded-lg border border-[#E5E7EB] bg-white">
      {(saving || msg) && (
        <p className={`px-4 py-2 text-[13px] ${msg ? "text-[#DC2626]" : "text-[#6B7280]"}`}>
          {msg ?? "排序保存中…"}
        </p>
      )}
      {sortable
...
function SortableRow(props: {
  ...
  return (
    <TableRow ref={setNodeRef} style={style} className="bg-white hover:bg-[#F9FAFB]">
      <TableCell className="w-8 pr-0">
        <button
          type="button"
          aria-label="拖动排序"
          className="flex h-7 w-7 cursor-grab items-center justify-center rounded text-[#9CA3AF] hover:bg-[#F3F4F6] active:cursor-grabbing"
          {...attributes}
          {...listeners}
        >
...
function RowCells({
  ...
  return (
    <>
      <TableCell className="max-w-[380px] truncate font-medium text-[#1F2937]">{row.title}</TableCell>
      {model === "knowledge" && (
        <TableCell>
          {row.category
            ? <span className="inline-flex items-center rounded-full border border-[#E5E7EB] bg-[#F9FAFB] px-2.5 py-0.5 text-xs text-[#6B7280]">{row.category}</span>
            : "—"}
        </TableCell>
      )}
      <TableCell><StatusChip status={chipStatus(row.status)} /></TableCell>
      {model === "intro" && (
        <TableCell className="text-[13px] text-[#6B7280]">{row.publishedAtText ?? "—"}</TableCell>
      )}
      <TableCell>

  ...
  return (
    <div className="mb-4 rounded-lg border border-border bg-card p-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-text-muted">运营可在此为本日补建时段（仅管理及以上）</span>
      </div>
      ...
      {open && (
        <div className="mt-4 grid grid-cols-2 gap-3 border-t border-muted pt-4 sm:grid-cols-3">
          <div className="space-y-1.5">
            <label htmlFor="slot-name" className="text-xs text-muted-foreground">时段名</label>
            <Input
              id="slot-name"
              value={form.name}
          ...
          <div className="space-y-1.5">
            <label htmlFor="slot-start" className="text-xs text-muted-foreground">开始时间</label>
            <Input
              id="slot-start"
              type="time"
          ...
          <div className="space-y-1.5">
            <label htmlFor="slot-end" className="text-xs text-muted-foreground">结束时间</label>
            <Input
              id="slot-end"
              type="time"
              value={form.endTime}
          ...
          {QUOTA_FIELDS.map(({ key, label }) => (
            <div key={key} className="space-y-1.5">
              <label htmlFor={`slot-${key}`} className="text-xs text-muted-foreground">{label}名额</label>
              <Input
                id={`slot-${key}`}
                type="number"
                min={0}

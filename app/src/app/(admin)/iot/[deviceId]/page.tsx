  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link href="/iot/devices" className="text-[13px] text-muted-foreground hover:text-foreground">
            ← 返回设备列表
          </Link>
        }
      />
      ...
      <div className="mb-5 rounded-lg border border-border bg-card p-5">
        <h2 className="mb-4 text-[14px] font-semibold text-foreground">设备基本信息</h2>
        {device ? (
          <dl className="grid grid-cols-2 gap-x-8 gap-y-3">
            {([
              ...
              <div key={label as string} className="flex flex-col gap-0.5">
                <dt className="text-[12px] text-muted-foreground">{label as string}</dt>
      ...
      <div className="rounded-lg border border-border bg-card">
        <div className="flex items-center justify-between border-b border-border px-5 py-3">
          <h2 className="text-[14px] font-semibold text-foreground">近 24 小时心跳数据</h2>
          <div className="flex items-center gap-2">
            <LiveDot tone={device?.status === "ONLINE" ? "online" : device?.status === "ALERT" ? "alert" : "offline"} />
            <span className="text-[12px] text-muted-foreground">心跳数据定时刷新</span>
          </div>
        </div>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {heartbeats.length === 0 ? (
              ...
              <TableRow key={hb.id} className="hover:bg-muted">
                <TableCell className="text-[13px] text-muted-foreground">{formatCnDateTime(hb.recordedAt)}</TableCell>

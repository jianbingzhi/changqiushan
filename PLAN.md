...
| 计划 | 标题 | 状态 | 备注 |
|---|---|---|---|
| [b-104](docs/plan/b-104-超宽融合指挥总屏实施计划.md) | 超宽融合指挥总屏（3840×1080 · `/screen/command`） | 🛠️ fix | 定稿 5 列版在 `deploy-demo`，随 b-105 合入 `main` |
| [b-105](docs/plan/b-105-单主干合并与收口.md) | `deploy-demo` 全量合回 `main` 与单主干收口 | 🔍 评审中 | 承接 ISSUES 的 N06 人工裁决（单主干）；含 N08、CI 触发分支、分支退役 |

> b-104b（融合大屏剩余执行清单）现只存在于 `deploy-demo` 分支，随 b-105 合并进入 `main` 后由 b-105 补登记本表。

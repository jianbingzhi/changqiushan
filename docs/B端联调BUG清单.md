...
- **严重度** 🔴 高（当天补录/在线预约链路再次瘫痪）　**状态** ✅ 已修（**2026-06-10 对齐**:b-102 落地规则派生+首单惰性物化+B34 读路径补完,当天时段正常派生可下单;收尾=b-103 T5 删废弃 cron 代码）　**页面** `/`、`/booking/slots`、`/booking/quota-rules`、`/booking/onsite`
- **现象**（线上 2026-06-08 实测）：
...
- **严重度** 🟡 中（违 PRD 红线 6：日期禁 ISO/英文，用「2026 年 6 月 8 日」）　**状态** ✅ 已修（**2026-06-10 对齐**:b-102 统一中文日期控件,analytics/onsite/quota-rules 无英文格式残留）　**页面** `/analytics/traffic`（开始/结束日期筛选）、`/booking/onsite`（预约日期），及其它用原生 `<input type=date>` 处
...
- **严重度** 🟠 高（PRD 1.2 核心规则层 + 已拍板 UX 未落地）　**状态** ✅ 已修（**2026-06-10 对齐**:b-102 落地 日历视图可切表格+派生惰性物化+总库存/单证单日上限;「立即生成」按钮废弃=b-103 T5 删除）　**页面** `/booking/quota-rules`（配额规则与日历）
...
- **严重度** 🟠 高（高频运营页,真实量级下不可用）　**状态** ✅ 已修（**2026-06-10 对齐**:b-102 落地服务端分页+分页器;Excel 导出缺口仍开放、非红线）　**页面** `/booking/bookings`（预约单查询）
...
> **〔2026-06-10 对齐〕** B 段(行政图下钻+地理热力)与 C 段(channels/iot/parking 只读 CRUD)已由 **b-102** 完成 ✅;D 段 seed 票务词=**b-103 T6**、E 段地图 UX=**b-103 T1**,均在执行中。

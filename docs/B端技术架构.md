...
> **设计要点·时段生成策略〔升级 2026-06-09，详见 `B端PRD需求覆盖对照.md` R-cfg〕**：
> 预约时段**不靠 cron 预先物化**，改为**规则派生 + 首单惰性物化**——定**基准**(常规日)叠加**节假日/特殊日特例**，查询某日时即时解析(无特例落基准、闭园=0)；`booking_slot` 行**仅在该日首笔预约时惰性创建**用于累加计数。
> 原因：cron 预生成是单点脆弱，**漏跑一次=当天全园静默无时段=事故**。新策略让"预约可用性"与定时任务解耦、自愈，cron 至多降级为可选预热。废弃 `rollGenerateSlots` 的预物化语义。

...
> 工程位置：小程序前端在 `miniprogram/`（自包含 Taro 工程），后端 BFF 在 `app/`。所有 `pnpm` 命令在 `miniprogram/` 下执行。
...
- `miniprogram/src/api/client.ts` 的 `API_BASE` 上线改为备案域名（当前联调指向 `http://localhost:3000/api/c`）。
- 微信支付回调 `WXPAY_NOTIFY_URL` 必须是备案 https 域名。

## 二、urlCheck（提审前开启）
`miniprogram/project.config.json` 当前 `setting.urlCheck = false`（仅联调期，便于开发者工具调本地 BFF）。
**提审前务必改回 `true`**，并用真机全链路回归（登录→预约→核销码→活动→AI→地图）确认无越权域名。

## 三、中文/红线 CI 扫描
- `cd miniprogram && pnpm lint:cn`（`miniprogram/scripts/lint-cn.mjs`）：扫描 `src` 全部 `.tsx`，命中 门票/票价/购票/退款/售票/票务/Lorem/ISO 日期 即失败。
- 接入 CI：在 PR 流水线加 `cd miniprogram && pnpm lint:cn && pnpm build:weapp`。
- 当前状态：✔ 无红线违规。

## 四、分包与体积治理
- 主包：A1–A6/A11 高频页（登录/首页/预约/我的）。
- 分包：`subpkg-activity`（A8/A9）、`subpkg-ai`（A7）、`subpkg-map`（A10）。
- `miniprogram/src/app.config.ts` 已配 `preloadRule`：首页预下载 `subpkg-activity`。
...
- `scope.userLocation`：已在 `miniprogram/src/app.config.ts` 配 `permission` 用途说明（园区导览定位）。

## 六、体验版上传（均在 `miniprogram/` 下执行）
